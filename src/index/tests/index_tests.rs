    ...
    let (start_0, end_0) = index
        ...
        .map(|it| {
            if let DataTypeInformation::Array { dimensions, .. } = it {
                ...
                (
                    dimensions[0].start_offset.get_int_value(&index).unwrap(),
                    dimensions[0].end_offset.get_int_value(&index).unwrap(),
                )
            } else {
                unreachable!()
            }
    ...
    let (start_1, end_1) = index
        ...
        .map(|it| {
            if let DataTypeInformation::Array { dimensions, .. } = it {
                //return the pair (start, end)
                (
                    dimensions[1].start_offset.get_int_value(&index).unwrap(),
                    dimensions[1].end_offset.get_int_value(&index).unwrap(),
                )
            } else {
                unreachable!()
            }
